...
#define QDEL	_delay_us(3)		// i2c quarter-bit delay
#define HDEL	_delay_us(6)		// i2c half-bit delay
...
void i2cswSend(u08 device, u08 subAddr, u08 length, u08 *data)
{
	...
	i2cswPutbyte(device); 		// send DEVICE address
	i2cswPutbyte(subAddr);		// and the subaddress
...
void i2cswReceive(u08 device, u08 subAddr, u08 length, u08 *data)
{
	...
	i2cswPutbyte(device | READ);	// resend DEVICE, with READ bit set
...
//Custom functions
//! Send a byte sequence on the I2C bus
void i2cswSendNS(u08 device, u08 length, u08 *data)
{
	I2C_START;      			// do start transition
	i2cswPutbyte(device); 		// send DEVICE address
...
//! Retrieve a byte sequence on the I2C bus
void i2cswReceiveNS(u08 device, u08 length, u08 *data)
{
	int j = length;
	u08 *p = data;
